...
def process_data(part_data):
    ...
    # heandler = ['brand', 'part_number', 'description', 'quantity', 'price_new', 'price_old','date' 'now','site']

    with lock:  # межпроцессная блокировка
        with open(f'{name_files}_exist.csv', 'a', newline='', encoding='utf-16') as file:
            writer = csv.writer(file, delimiter='\t')

            quantity = 50
            for item in part_data[:1]:  # используем part_data
                            ...
                            if match:
                                ...
                                if aggregated_parts:
                                    ...
                                    if statistic_html:
                                        ...
                                        data_transport = match.group(1) if match else None

                                aggregated_parts = json_data[0].get('AggregatedParts', [])
                                if aggregated_parts:
                                    lowest_price = aggregated_parts[0].get('priceString', None)
                                else:
                                    lowest_price = None
