...
def get_content(url):
    ...
    try:
        ...
        # group = url.split("/")[-1]
        # with open(f"C:\\scrap_tutorial-master\\temp\\{group}.csv", "w", errors='ignore') as file:
        ...
        button_lang = driver.find_element(By.XPATH, '//div[@class="lang"]').click()
        driver.implicitly_wait(5)
        table = driver.find_element(By.XPATH, '//div[@id="medicamentSearchForm:results"]//div[@class="backgroundwhite"][1]')
        table_header = driver.find_elements(By.XPATH, '//div[@id="medicamentSearchForm:results"]//div[@class="backgroundwhite"][1]//th[@class="rf-dt-hdr-c"]')
        for item_header in table_header:
            table_header_list.append(item_header.text)
        product_list_one = driver.find_element(By.XPATH, '//tbody[@id="medicamentSearchForm:resultRegisterOneTable:tb"]//tr[@class="rf-dt-r rf-dt-fst-r"]').text
        ...
        with open(f"C:\\scrap_tutorial-master\\temp\\test.csv", "w", errors='ignore') as file:
            writer = csv.writer(file, delimiter=";", lineterminator="\r")
            writer.writerow(
                (
                    table_header_list
        ...
        #                                           f'//li[@data-position="{i}"]//li[@class="website-link website-link-a"]//a').get_attribute(
        #                 "href")
        #             product_url.append(
        #                 {company.split("/")[-2]: company}
        ...
        #         driver.implicitly_wait(5)
        #         next_button = driver.find_element(By.XPATH, '//li[@class="page-item next]')
...
def parse_content():
    url = "https://portal.ncpr.bg/registers/pages/register/list-medicament.xhtml"
    get_content(url)
