...
def save_link_all_product(url):
    ...
    for item in card_product_url[1:]:
        product_url.append(
            {
    ...
    with open(f"C:\\lidl\\card_url.json", 'w', encoding="utf-8") as file:
...
def parsing_product():
    with open(f"C:\\lidl\\card_url.json") as file:
        all_site = json.load(file)
    product_url = []
    data = []
    char_prod_all = {}
    with open(f"C:\\lidl\\product.csv", "w", errors='ignore') as file:
    ...
    for item in all_site:
        ...
        try:
            ...
            price = 'No price'
        with open(f"C:\\lidl\\product.csv", "a", errors='ignore') as file:
            writer = csv.writer(file, delimiter=";", lineterminator="\r")
            writer.writerow(
                (
                    name_product,
