...
def get_url_category(url):
    ...
    match = re.search(r'About (\d+) results', about_results)
    if match:
        result_number = int(match.group(1))
    else:
        print("Числа в тексте не найдены!")
    ...
    for file in folders_html:
        ...
        for item in files_json:
            ...
            card_product = soup.find('div', attrs={'class': 'row q-col-gutter-sm-md q-px-sm-md'}).find_all('div',
                                                                                                           attrs={
                                                                                                               'class': 'full-height full-width position-relative'})
        ...
        for u in urls[:5]:
            ...
            try:
                if driver.find_element(By.XPATH, '//div[@class="precheckout__title precheckout__offer-title"]'):
                    ...
                    price_server = driver.find_element(By.XPATH, '//span[@id="displayPrice"]').text.replace(".", ",")
                    currency_server = driver.find_element(By.XPATH, '//span[@class="text-currency"]').text
                    ...
                    try:
                        stock = driver.find_element(By.XPATH, '//input[@class="stock-box__count"]').get_attribute(
                            'value')
                    except:
                        stock = ""
                    try:
                        checkout_devlivery = driver.find_element(By.XPATH,
                                                                 '//div[@class="available-section"]//div').text
                    except:
                        checkout_devlivery = ""
                    products_all.append(
                        {
                            'price': price_server,
                            'currency_server': currency_server,
