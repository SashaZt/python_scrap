...
def save_url_csv():
    """mezczyzni"""
    urls_mezczyzni = "https://outlet.asics.com/pl/pl-pl/mens-gear/c/ao10000000/?sz=96"
    response = requests.get(urls_mezczyzni, headers=header)  # , proxies=proxies
    soup = BeautifulSoup(response.text, 'lxml')
    totalResources_mezczyzni = 0
    if response.status_code == 200:
        results_tag = soup.find('span', class_='results-hits pagination__results medium-reg')
        results_count = int(results_tag.text.strip('()\n'))
        totalResources_mezczyzni = results_count // 96
    counter_mezczyzni = 0
    links_mezczyzni = []

    for page in range(1, totalResources_mezczyzni + 2):
        if page == 1:
            url_mezczyzni_api = 'https://outlet.asics.com/pl/pl-pl/mens-gear/c/ao10000000/?sz=96'
        else:
            counter_mezczyzni += 96
            url_mezczyzni_api = f'https://outlet.asics.com/pl/pl-pl/mens-gear/c/ao10000000/?start={counter_mezczyzni}&sz=96'
    ...
    with open(f'c:\\outlet_asics_com\\csv_url\\mens-gear\\url.csv', 'w', newline='', encoding='utf-8') as csvfile:
    ...
    """womens"""
    urls_womens = "https://outlet.asics.com/pl/pl-pl/womens-gear/c/ao20000000/?sz=96"
    ...
    if response.status_code == 200:
        results_tag = soup.find('span', class_='results-hits pagination__results medium-reg')
        results_count = int(results_tag.text.strip('()\n'))
        totalResources_womens = results_count // 96
    counter_womens = 0
    links_womens = []

    for page in range(1, totalResources_womens + 2):
        if page == 1:
            url_womens_api = 'https://outlet.asics.com/pl/pl-pl/womens-gear/c/ao20000000/?sz=96'
        else:
            counter_womens += 96
            url_womens_api = f'https://outlet.asics.com/pl/pl-pl/womens-gear/c/ao20000000/?start={counter_womens}&sz=96'
    ...
    with open(f'c:\\outlet_asics_com\\csv_url\\womens-gear\\url.csv', 'w', newline='', encoding='utf-8') as csvfile:
        writer = csv.writer(csvfile, delimiter='\n', quotechar='|')
    ...
    """kids"""
    urls_kids = "https://outlet.asics.com/pl/pl-pl/kids-gear/c/ao30000000/?sz=96"
    ...
    if response.status_code == 200:
        results_tag = soup.find('span', class_='results-hits pagination__results medium-reg')
        results_count = int(results_tag.text.strip('()\n'))
        totalResources_kids = results_count // 96
    counter_kids = 0
    links_kids = []

    for page in range(1, totalResources_kids + 2):
        if page == 1:
            url_kids_api = 'https://outlet.asics.com/pl/pl-pl/kids-gear/c/ao30000000/?sz=96'
        else:
            counter_kids += 96
            url_kids_api = f'https://outlet.asics.com/pl/pl-pl/kids-gear/c/ao30000000/?start={counter_kids}&sz=96'
    ...
    with open(f'c:\\outlet_asics_com\\csv_url\\kids-gear\\url.csv', 'w', newline='', encoding='utf-8') as csvfile:
        writer = csv.writer(csvfile, delimiter='\n', quotechar='|')
...
def drop_duplicates():
    all_csv = ['c:\\outlet_asics_com\\csv_url\\kids-gear\\url.csv',
               'c:\\outlet_asics_com\\csv_url\\womens-gear\\url.csv',
               'c:\\outlet_asics_com\\csv_url\\mens-gear\\url.csv']
    for f in all_csv:
        ...
        df_unique.to_csv(f'c:\\outlet_asics_com\\csv_url\\{value}\\url.csv', index=False)
...
def parsin_contact_html():
    folders_html = [r"c:\outlet_asics_com\html_product\kids-gear\*.html",
               r"c:\outlet_asics_com\html_product\mens-gear\*.html",
               r"c:\outlet_asics_com\html_product\womens-gear\*.html"
               ]
    ...
    for file in folders_html: # Убарть срез для категории!
        group = file.split('\\')[3]
        with open(f"c:\\outlet_asics_com\\csv_data\\{group}.csv", "w",
        ...
        for item in files_json: # Убарть срез для файла!
            ...
            for script in scripts[2:3]: # оставить т.к. на странице несколько скриптов
                ...
                Title = "ASICS Outlet " + utag_data.get('product_name')[0]
                ...
                color = ''
                try:
                    color_old = utag_data.get('product_variant')[0].split('/')[0]
            ...
            for coun, img in enumerate(alls_photo, 1):
                ...
                if img_group_folder:
                    pfoto_site.append(
                        f"https://loketsneakers.s3.eu-central-1.amazonaws.com/asics_outlet/{img_group_folder}/{id_product}_{coun}.webp")
                    file_path = f"asics_outlet/{img_group_folder}/{id_product}_{coun}.webp"
                    ...
                    # pfoto_site.append(
                    #     f"https://loketsneakers.s3.eu-central-1.amazonaws.com/asics_outlet/{img_group_folder}/{id_product}_{coun}.webp")
                    # file_path = f"c:\\outlet_asics_com\\data_img\\{img_group_folder}\\{id_product}_{coun}.webp"
                    # new_file_path = f"c:\\outlet_asics_com\\csv_data\\{img_group_folder}\\{id_product}_{coun}.webp"
            ...
            #
            #     file_path = f"c:\\outlet_asics_com\\data_img\\img_{group}\\{id_product}_{coun}.webp"
            #     new_file_path = f"c:\\outlet_asics_com\\csv_data\\img_{group}\\{id_product}_{coun}.webp"
            #     if not os.path.exists(file_path):
            #         img_data = requests.get(img, headers=header, proxies=proxies)
            #         with open(new_file_path, 'wb') as file_img:
            #             file_img.write(img_data.content)
            ...
            for i, size in enumerate(sizes):
                ...
                data = [Handle, Title, "", Vendor, 'Apparel & Accessories > Shoes', category_product,
                        ...
                        "",
                        "", "", "", "", "", "", "", "", "", "", "kg", "", "", "TRUE", "TRUE", "", "", "Active"]
                with open(f"c:\\outlet_asics_com\\csv_data\\{group}.csv", "a",
...
def move_img():
    folders = {
        r'c:\outlet_asics_com\csv_data\img_kids-gear': r'c:\outlet_asics_com\data_img\img_kids-gear',
        r'c:\outlet_asics_com\csv_data\img_mens-gear': r'c:\outlet_asics_com\data_img\img_mens-gear',
        r'c:\outlet_asics_com\csv_data\img_womens-gear': r'c:\outlet_asics_com\data_img\img_womens-gear'
    }
...
def del_files_html_product():
    folders_del = [r"c:\outlet_asics_com\html_product\kids-gear",
               r"c:\outlet_asics_com\html_product\mens-gear",
               r"c:\outlet_asics_com\html_product\womens-gear"
               ]
