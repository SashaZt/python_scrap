...
async def get_page(session, url, category):
    ...
    async with session.get(url, headers=header, proxy=proxy, proxy_auth=proxy_auth) as r: #
        if r.status == 200:
            ...
            bad_url.append(url)
            with open(f'c:\\nike_com_pl\\csv_url\\{category}\\bad_url.csv', "a",
...
async def parse(results, counter, category):
    for i in results:
        if i != None:
            counter += 1
            with open(
                    f"c:\\nike_com_pl\\html_product\\{category}\\0_{counter}.html",
...
async def main(category):
    async with aiohttp.ClientSession() as session:
        with open(f'c:\\nike_com_pl\\csv_url\\{category}\\url.csv', newline='', encoding='utf-8') as files:
        ...
        if os.path.isfile(f'c:\\nike_com_pl\\csv_url\\{category}\\bad_url.csv'):
            new_filename = f'c:\\nike_com_pl\\csv_url\\{category}\\url.csv'
            if os.path.isfile(new_filename):
                os.remove(new_filename)
            os.rename(f'c:\\nike_com_pl\\csv_url\\{category}\\bad_url.csv', new_filename)
            await main(category)
            """
...
async def main(category):
    ...
    while retry_count < 3:
        async with aiohttp.ClientSession() as session:
            with open(f'c:\\nike_com_pl\\csv_url\\{category}\\url.csv', newline='',
