...
def get_requests():
    response = requests.get('https://www.binance.com/fapi/v1/ticker/24hr?symbol=BTCUSDT', cookies=cookies, headers=headers,
                            proxies=proxies)
    src = response.text
    soup = BeautifulSoup(src, 'html.parser')  # Создаем объект BeautifulSoup для парсинга HTML
    ...
    print('Quote Volume:', quoteVolume)
