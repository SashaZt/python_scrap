...
import requests
from main_asio import run_categories
...
def save_html_product():
    ...
    all_urls = ["https://nbsklep.pl/meskie/obuwie", "https://nbsklep.pl/damskie/obuwie","https://nbsklep.pl/dzieciece/obuwie"]
    for u in all_urls:
        group = u.split("/")[-2]
        ...
        for filename in os.listdir(dir_path):
            ...
            if os.path.isfile(file_path):
                os.remove(file_path)
        coun = 0
        resp = requests.get(u, headers=header, proxies=proxies)
        ...
        try:
            ...
            data_json_1 = ""
        lastPage = int(
            data_json_1['props']['pageProps']['dehydratedState']['queries'][7]['state']['data']['products']['pagination'][
                'lastPage'])
        urls = []
        for i in range(1, lastPage + 1):
            ...
            if i > 1:
                resp = requests.get(f'{u}?page={i}', headers=header, proxies=proxies)
                resp.raise_for_status()
                soup = BeautifulSoup(resp.text, 'lxml')
    ...
    for folder in folders:
        ...
        for item in files_html:
                ...
                try:
                    ...
                    Handle = (data_json['props']['pageProps']['dehydratedState']['queries'][5]['state']['data']['product']['name'])
                    last_word_Handle = Handle.split()[-1]  # выбрать последнее слово
                    Handle_new = ' '.join(["New Balance", last_word_Handle])
                except:
                    print("Ошибка в Handle")
                    continue
                Title = (data_json['props']['pageProps']['dehydratedState']['queries'][5]['state']['data']['product']['name']).replace('–', '-').split('-')[0].strip()
                last_word_Title = Title.split()[-1]  # выбрать последнее слово
                Title_new = ' '.join(["New Balance", last_word_Title])
                Vendor = (
                data_json['props']['pageProps']['dehydratedState']['queries'][5]['state']['data']['product']['producer'][
                    'name'])
                Tags = (
                data_json['props']['pageProps']['dehydratedState']['queries'][3]['state']['data']['menu']['children'][1][
                ...
                if sell_price < base_price:
                    ...
                    Variant_Compare_At_Price = round(((float(base_price) + 60.0) *1.1) * bel, 1) # два знака после запятой
                ...
                id_product = data_json['props']['pageProps']['dehydratedState']['queries'][5]['state']['data']['product']['id']
                ...
                for item in data_size:
                    if item["availability"]["message"]["content"] == "Dostępny":
                        sizes.append(item["option"])
                """Размеры"""

                category_product = \
                data_json['props']['pageProps']['dehydratedState']['queries'][5]['state']['data']['product']['categories'][0][
                    'name']

                picture_dicts = data_json['props']['pageProps']['dehydratedState']['queries'][5]['state']['data']['product'][
                ...
                for img in alls_photo:
                    ...
                    file_path = f"c:\\nbsklep_pl\\data_img\\img_{group}\\{id_product}_{coun}.webp"
                    new_file_path = f"c:\\nbsklep_pl\\csv_data\\img_{group}\\{id_product}_{coun}.webp"
                ...
                """Предварительно ХОРОШИЙ ТЕСТОВЫЙ Вариант"""
                ...
                for i, size in enumerate(sizes):
                    ...
                    data = [Handle, Title_new, "", Vendor, 'Apparel & Accessories > Shoes', 'Shoes', category_product, 'TRUE',
                ...
                if len(pfoto_site) > len(sizes):
                    ...
                    for picture_dict in picture_dicts:
                        alls_photo.append(
                            picture_dict['filename'].replace('https://nbsklep.pl/{imageSafeUri}', 'https://nbsklep.pl/picture'))
                    for j in range(len(sizes), len(pfoto_site)):
                        photo = pfoto_site[j]
                        index_photo_str = str(index_photo)  # convert index to string
                        data = [Handle, Title_new, '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '',
...
def creating_an_archive():
    directory_to_zip = "c:\\nbsklep_pl\\csv_data"
    DATE_FORMAT = "%d_%m_%y"
    zipfile_name = datetime.now().strftime(DATE_FORMAT) + ".7z"

    try:
        # создаем объект Py7zr для записи архива
        with py7zr.SevenZipFile(f"c:\\nbsklep_pl\\{zipfile_name}", mode='w') as archive:
            # рекурсивно проходим по всем файлам в папке и добавляем их в архив
            for root, dirs, files in os.walk(directory_to_zip):
                for filename in files:
                    file_path = os.path.join(root, filename)
                    archive.write(file_path, os.path.relpath(file_path, directory_to_zip))
        print("Archive created successfully!")
    except Exception as e:
        print("Error creating archive:", e)

def move_img():
    # определить исходную и целевую папки
    src_damskie = r'c:\nbsklep_pl\csv_data\img_damskie'
    dest_damskie = r'c:\nbsklep_pl\data_img\img_damskie'
    src_dzieciece = r'c:\nbsklep_pl\csv_data\img_dzieciece'
    dest_dzieciece = r'c:\nbsklep_pl\data_img\img_dzieciece'
    src_meskie = r'c:\nbsklep_pl\csv_data\img_meskie'
    dest_meskie = r'c:\nbsklep_pl\data_img\img_meskie'
    # перебрать все файлы и подпапки в исходной папке
    for root, dirs, files in os.walk(src_damskie):
        # определить путь к подпапке в целевой папке
        dest_root = root.replace(src_damskie, dest_damskie)
        for file in files:
            src_path = os.path.join(root, file)
            dest_path = os.path.join(dest_root, file)
            shutil.move(src_path, dest_path)
    for root, dirs, files in os.walk(src_dzieciece):
        # определить путь к подпапке в целевой папке
        dest_root = root.replace(src_dzieciece, dest_dzieciece)
        for file in files:
            src_path = os.path.join(root, file)
            dest_path = os.path.join(dest_root, file)
            shutil.move(src_path, dest_path)
    for root, dirs, files in os.walk(src_meskie):
        # определить путь к подпапке в целевой папке
        dest_root = root.replace(src_meskie, dest_meskie)
        for file in files:
            src_path = os.path.join(root, file)
            dest_path = os.path.join(dest_root, file)
            shutil.move(src_path, dest_path)
