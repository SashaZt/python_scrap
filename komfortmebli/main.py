...
def get_content(url):
    ...
    try:
        url_table_all_categories = []
        url_firma = []
        resp = requests.get(url, headers=header)
        soup = BeautifulSoup(resp.text, 'lxml')
        table_all_categories = soup.find('nav', class_='bx-top-nav-container').find_all('li')

        for i in table_all_categories[28:]:
            url_table_all_categories.append('https://komfortmebli.com.ua' + i.find_next('a').get("href"))
        for j in url_table_all_categories[0:1]:
