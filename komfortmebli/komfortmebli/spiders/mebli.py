...
from komfortmebli.items import Product
...
class MebliSpider(scrapy.Spider):
    ...
    def parse_item(self, response, **kwargs):
        item = Product()
        item['price_old'] = response.xpath('//div[@class="product-item-detail-price-old"]/text()').get().replace('\n',
                                                                                                                 '').replace(
            '\t', '').replace('\r', '').strip()
        item ['title'] = response.xpath('//div[@class="container-fluid"]//h1/text()').get()

        # item ['price_new'] = response.xpath('//div[@class="product-item-detail-price-current"]/text()').get().replace('\n',
        #                                                                                                      '').strip()
        # item ['des'] = response.xpath('//div[@class="product-item-detail-tab-content active"]//b/text()').getall()
        yield item
