...
def get_url_category(url):
    ...
    # url_img = driver.find_elements(By.XPATH,
    #                                '//div[@class="xdt5ytf x193iq5w x6ikm8r x1odjw0f"]//div[@class="xfitlp5 x1n2onr6 xh8yej3"]//img')
    # for u in url_img:
    #     href = u.get_attribute("src")
    #     urls_card.append(
    #         {
    #             'url_name': href
    #         }
    ...
    with open(f"urls_card.json") as file:
        all_site = json.load(file)
    counter = 0
    for item in all_site:
        counter += 1
        time.sleep(1)
        img_data = requests.get(item['url_name'])
        with open(f"{counter}.jpg", 'wb') as file_img:
...
def parse_content():
    url = "https://www.facebook.com/media/set/?set=oa.664968387336642&type=3"
