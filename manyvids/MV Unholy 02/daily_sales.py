...
def get_id_models():
    ...
    cursor.execute("SELECT DISTINCT model_fm, mvtoken FROM manyvids_02.daily_sales")
...
def get_sql_check_chat():
    ...
    cursor = cnx.cursor()
    cursor.execute("""
        SELECT msg_last_id, CONCAT(date_part, ' ', time_part) AS datetime FROM manyvids_02.chat;
    """)
...
def get_requests(month, filterYear):
    ...
    for item in files_json:
        ...
        while attempts < max_attempts:
            try:
                proxi = proxy_random()
                response_day = session.post('https://www.manyvids.com/includes/get_earnings.php',
                                            headers=headers,
                                            proxies=proxi,
                                            data=data_day)
                ...
                print('Парсим дневные')
                break  # Если запрос успешен, выходим из цикла
            except requests.exceptions.ConnectionError as e:
                print(f"Попытка {attempts + 1} не удалась из-за проблемы соединения: {e}")
        ...
        while attempts < max_attempts:
            try:
                proxi = proxy_random()
                response_pending_custom = session.get('https://www.manyvids.com/View-my-earnings/', headers=headers, proxies=proxi)
                print('Парсим pending')
        ...
        while attempts < max_attempts:
            try:
                proxi = proxy_random()
                response_chat = session.get('https://www.manyvids.com/includes/user_messages.php',
                                       params=param_chat,
                                       headers=headers,
                                       proxies=proxi)
                if response_chat.status_code == 200:
                    print('Парсим user_messages')
                    break  # Если запрос успешен, выходим из цикла
                else:
                    print(f"Попытка {attempts + 1} не удалась: HTTP статус {response_chat.status_code}")
            except requests.exceptions.ConnectionError as e:
                print(f"Попытка {attempts + 1} не удалась из-за проблемы соединения: {e}")
            except requests.exceptions.RequestException as e:
                print(f"Попытка {attempts + 1} не удалась: {e}")
        ...
        try:
            json_data = response_chat.json()
        except json.JSONDecodeError:
            print(f'что-то не то с {mvtoken}')
            continue
        ...
        try:
            ...
            print('Не удалось извлечь общее количество сообщений')
            continue
...
def get_sql_data_data_day():
    ...
    cursor = cnx.cursor()
    cursor.execute("""
        SELECT buyer_user_id, sales_date, sales_time, seller_commission_price FROM manyvids_02.daily_sales;
    """)
...
def get_sql_data_payout_history():
    ...
    cursor = cnx.cursor()
    cursor.execute("""
        SELECT model_id,payment_date,paid  FROM manyvids_02.payout_history;
    """)
...
def get_pending_to_google():
    ...
    clear_pending_query = """
            UPDATE manyvids_02.monthly_sales
    ...
    query = """
        ...
        FROM manyvids_02.monthly_sales
...
ORDER BY 
    ...
    sheet_payout_history.update([[current_datetime]], 'A1')
    # Получаем список всех файлов в папке
    files = glob.glob(os.path.join(payout_history_path, '*'))
    # Удаляем каждый файл
    for f in files:
        if os.path.isfile(f):
            os.remove(f)
